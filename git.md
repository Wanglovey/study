...
当在git中输入`git log`类型的指令后，git会显示出历史提交记录相关信息，如下图所示

![image-20220408150012605](C:\Users\ASUS\AppData\Roaming\Typora\typora-user-images\image-20220408150012605.png)

按**q键**可退出当前界面

### 未添加备注

如果在git中未添加备注提交到仓库的话，会出现如下界面

![image-20220408150111602](C:\Users\ASUS\AppData\Roaming\Typora\typora-user-images\image-20220408150111602.png)

之后的操作步骤如下

1. 点击**i键**进入插入模式
2. 在插入模式下添加提交信息。如果不想添加任何提交信息可以输入`#`号，表示注释
3. 添加本次提交信息后点击`esc键`退出insert模式
4. 在界面中输入`:wq`完成本次提交并退出注释界面

### 克隆远程仓库

步骤如下

1. 在git中输入`git clone 远程仓库地址`，将远程仓库的主分支克隆到当前目录下
2. 在当前目录创建与远程仓库一一对应的分支名称
3. 在各个分支下，使用`git push -u origin 分支名称`一一建立与远程仓库的联系
4. 在各个分支下使用`git pull`拉取分支的最新信息

> 以上操作略微繁琐，如果有更好的方法可以联系我
...
|     `git push -u 远程仓库名 本地分支名:远程分支名`     | 建立本地分支与远程仓库分支的联系，如果本地分支名不写则默认为当前分支建立与远程仓库分支的联系 |
|                       `git push`                       | 上方命令使用过一次后才可以使用此命令。<br/>将本地仓库推送到git仓库 |
|                        克隆仓库                        |                                                              |
