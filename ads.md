...
| ---- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| 1    | 新节点即根节点                                               | 将新节点变色为黑色                                           |
| 2    | 新节点的父节点是黑色                                         | 将插入的新节点设为红色即可                                   |
| 3    | 新节点的父节点和叔节点都是红色                               | 将新节点的父节点和叔节点变为黑色，将新节点的祖父节点变为红色，使得局部满足红黑树规则，之后从祖父节点开始依次检查上去，直到整棵树均满足红黑树规则 |
| 4    | 新节点的父节点是红色，叔节点是黑色或者没有叔节点，且祖父节点到新节点的关系为左右 | 以父节点为轴，做一次左旋，变为情况5，然后根据情况5进行处理即可 |
| 5    | 新节点的父节点是红色，叔节点是黑色或者没有叔节点，且祖父节点到新节点的关系为左左 | 以祖父节点为轴，进行一次右旋，使原先的父节点成为祖父节点，然后将原来的祖父节点变为黑色，将原来的父节点变为红色 |
| 6    | 右左，为情况4的镜像                                          | 略                                                           |
| 7    | 右右，为情况5的镜像                                          | 略                                                           |
...
Precision P = R~R~ / (R~R~ + I~R~)

Recall    R = R~R~ / (R~R~ + R~N~)
...
- 定理
  - 一个具有r个节点在右路径上的最左树一定有至少2^r^-1个节点（可用归纳法证明）
